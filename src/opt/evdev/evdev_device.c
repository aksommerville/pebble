...
int evdev_device_update(struct pblrt_input *driver,struct evdev_device *device) {
  ...
  if (driver->delegate.cb_button) {
    ...
    for (;eventc-->0;event++) {
      ...
      driver->delegate.cb_button(device->devid,(event->type<<16)|event->code,event->value);
    }
  }
  return 0;
}
...
static int evdev_for_each_bit(
  ...
  for (;major<bitsc;major++) {
    ...
    for (;minor<8;minor++,mask<<=1) {
      ...
      int pblbtnid=evdev_guess_pblbtnid(type,code);
      int hidusage=evdev_guess_hidusage(type,code);
      if (err=cb((type<<16)|code,pblbtnid,hidusage,lo,hi,value,userdata)) return err;
    }
  }
  return 0;
}
