 ...
 * This is copied from xegl/xegl_render.c, and should be exactly the same.
 */
...
static int drmgx_render_program_compile(
...
static int drmgx_render_program_init(
  struct pblrt_video *driver,
...
int drmgx_render_init(struct pblrt_video *driver) {
  ...
  if (!(DRIVER->program=drmgx_render_program_init(
    driver,
    drmgx_vsrc,sizeof(drmgx_vsrc)-1,
    drmgx_fsrc,sizeof(drmgx_fsrc)-1
  ))) return -1;
...
static void drmgx_render_calculate_bounds(struct pblrt_video *driver,int fbw,int fbh) {
...
int drmgx_render_commit(struct pblrt_video *driver,const void *fb,int fbw,int fbh) {
  if ((fbw<1)||(fbw>DRMGX_FRAMEBUFFER_SIZE_LIMIT)) return -1;
  if ((fbh<1)||(fbh>DRMGX_FRAMEBUFFER_SIZE_LIMIT)) return -1;
  ...
  glVertexAttribPointer(0,2,GL_SHORT,0,sizeof(struct drmgx_vertex),&vtxv[0].x);
  glVertexAttribPointer(1,2,GL_FLOAT,0,sizeof(struct drmgx_vertex),&vtxv[0].tx);
