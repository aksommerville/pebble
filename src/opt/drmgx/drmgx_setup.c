...
static int drmgx_select_best_mode(struct pblrt_video *driver,drmModeResPtr res) {
  //fprintf(stderr,"%s...\n",__func__);
  ...
  for (;conni<res->count_connectors;conni++) {
    ...
    for (;modei<conn->count_modes;modei++,mode++) {
      ...
      if (drmgx_pick_mode(driver,bestconn,&bestmode,conn,mode)>0) {
...
int drmgx_configure(struct pblrt_video *driver) {
  ...
  if (drmgx_select_best_mode(driver,res)<0) {
