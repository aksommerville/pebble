 ...
 * We don't touch EGL or VideoCore.
 */
...
static int bcm_render_program_compile(
...
static int bcm_render_program_init(
  struct pblrt_video *driver,
...
int bcm_render_init(struct pblrt_video *driver) {
  ...
  if (!(DRIVER->program=bcm_render_program_init(
    driver,
    bcm_vsrc,sizeof(bcm_vsrc)-1,
    bcm_fsrc,sizeof(bcm_fsrc)-1
  ))) return -1;
...
static void bcm_render_calculate_bounds(struct pblrt_video *driver,int fbw,int fbh) {
...
int bcm_render_commit_framebuffer(struct pblrt_video *driver,const void *fb,int fbw,int fbh) {
  ...
  glVertexAttribPointer(0,2,GL_SHORT,0,sizeof(struct bcm_vertex),&vtxv[0].x);
  glVertexAttribPointer(1,2,GL_FLOAT,0,sizeof(struct bcm_vertex),&vtxv[0].tx);
